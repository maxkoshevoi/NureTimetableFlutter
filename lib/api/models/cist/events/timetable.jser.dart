...
abstract class _$TimetableSerializer implements Serializer<Timetable> {
  ...
  Serializer<Event> get _eventSerializer => __eventSerializer ??= EventSerializer();
  ...
  Serializer<Teacher> get _teacherSerializer => __teacherSerializer ??= TeacherSerializer();
  Serializer<Lesson> __lessonSerializer;

  Serializer<Lesson> get _lessonSerializer => __lessonSerializer ??= LessonSerializer();
  Serializer<EventType> __eventTypeSerializer;

  Serializer<EventType> get _eventTypeSerializer => __eventTypeSerializer ??= EventTypeSerializer();
  ...
  Map<String, dynamic> toMap(Timetable model) {
    ...
    setMapValueIfNotNull(ret, 'events',
        codeNonNullIterable(model.events, (val) => _eventSerializer.toMap(val as Event), []));
    ...
    setMapValueIfNotNull(ret, 'teachers',
        codeNonNullIterable(model.teachers, (val) => _teacherSerializer.toMap(val as Teacher), []));
    setMapValueIfNotNull(ret, 'subjects',
        codeNonNullIterable(model.lessons, (val) => _lessonSerializer.toMap(val as Lesson), []));
    setMapValueIfNotNull(
        ret,
        'types',
        codeNonNullIterable(
            model.eventTypes, (val) => _eventTypeSerializer.toMap(val as EventType), []));
    return ret;
  }
  ...
  Timetable fromMap(Map map) {
    ...
    final obj = Timetable(
        ...
        groups:
            codeNonNullIterable<Group>(map['groups'] as Iterable, (val) => _groupSerializer.fromMap(val as Map), <Group>[]) ??
