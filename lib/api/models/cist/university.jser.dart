...
abstract class _$UniversitySerializer implements Serializer<University> {
  ...
  Serializer<Faculty> get _facultySerializer => __facultySerializer ??= FacultySerializer();
  Serializer<Building> __buildingSerializer;

  Serializer<Building> get _buildingSerializer => __buildingSerializer ??= BuildingSerializer();
  ...
  Map<String, dynamic> toMap(University model) {
    ...
    setMapValueIfNotNull(
        ...
        'faculties',
        codeNonNullIterable(
            model.faculties, (val) => _facultySerializer.toMap(val as Faculty), []));
    setMapValueIfNotNull(
        ret,
        'buildings',
        codeNonNullIterable(
            model.buildings, (val) => _buildingSerializer.toMap(val as Building), []));
    return ret;
  }
  ...
  University fromMap(Map map) {
    ...
    final obj = University(
        ...
        buildings: codeNonNullIterable<Building>(map['buildings'] as Iterable,
                (val) => _buildingSerializer.fromMap(val as Map), <Building>[]) ??
